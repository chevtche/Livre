...
void TransferFunction1DRaw::reset()
{
    ...
    for( size_t i = 0; i < size; ++i )
        _data[i] = maxVal * defaultTF[i];
}

void TransferFunction1DRaw::createCustomTF_( const uint32_t size )
{
    _data.resize( size * TF_NCHANNELS );
    ...
    {
        _data[ i * TF_NCHANNELS ] = 0u;
        _data[ i * TF_NCHANNELS + 1 ] = 0u;
        _data[ i * TF_NCHANNELS + 2 ] = 255u;
        _data[ i * TF_NCHANNELS + 3 ] = 2u;
    }
}

void TransferFunction1DRaw::createTfFromFile_( const std::string& file )
{
    FloatVector transferFunction;
    ...
    for( size_t i = 0; i < size; ++i )
        _data[i] = conversionFactor * transferFunction[i];
}
